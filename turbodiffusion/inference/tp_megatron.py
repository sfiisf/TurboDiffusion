...
from rcm.networks.wan2pt2 import WanSelfAttention, ParallelWanSelfAttention
...
from rcm.networks.wan2pt2 import ParallelWanSelfAttention, WanSelfAttention, WanCrossAttention
...
def load_model(model_path):
    ...
    replace = {}
    for name, module in net.blocks.named_modules():
        if isinstance(module, WanSelfAttention) and not isinstance(module, WanCrossAttention):
            replace[name] = ParallelWanSelfAttention.from_wan_self_attention(module, parallel_size=world_size)
    for name, new_module in replace.items():
        parent_module = net.blocks
